      ...
      {(title || description || action) ? (
        <div className="mb-5 flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            {title ? <h3 className="break-words text-lg font-semibold tracking-tight text-[color:var(--text-strong)] sm:text-xl">{title}</h3> : null}
            {description ? <p className="max-w-2xl text-sm leading-6 text-[color:var(--text-soft)]">{description}</p> : null}
          </div>
